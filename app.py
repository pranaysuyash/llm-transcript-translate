...
def transcribe():
    ...
    try:
        ...
        if processed_duration is not None:
            logger.info(
                f"Processed Duration: {processed_duration}s, Video Duration: {video_duration}s"
            )
            if processed_duration < video_duration:
                logger.warning(
                ...
                return (
                    jsonify(
                        {
                            "error": "Transcription incomplete",
            ...
            # If we couldn't find processed duration in the output
            return (
                jsonify(
                    {
                        "error": "Could not determine processed duration",
                        "video_duration": video_duration,
                        "output": result,
                    }
                ),
